...
initialize_table_rhcNodesTable(void)
{
	...
	if (!my_handler || !table_info || !iinfo) {
		snmp_log(LOG_ERR, "malloc failed in initialize_table_rhcNodesTable");
						...
						netsnmp_request_info *requests)
{
	...
	for (request = requests ; request ; request = request->next) {
		...
		switch (reqinfo->mode) {
			...
			default:
				/* == the other SET modes */
				ci = (struct commitInfo*)
						netsnmp_oid_stash_get_data(commitStorage,
							 (oid *) ((char *) suffix + 1), suffix_len - 1);
				break;
		}
		...
		switch (reqinfo->mode) {
			case MODE_GET:
				switch (table_info->colnum) {
					...
					case COLUMN_RHCNODESTATUSCODE:
					{
						long *retval;
						size_t retval_len = 0;
						retval = get_rhcNodeStatusCode(data_context,
									&retval_len);
					...
					case COLUMN_RHCNODESTATUSDESC:
					{
						char *retval;
						size_t retval_len = 0;
						retval = get_rhcNodeStatusDesc(data_context,
									&retval_len);
						snmp_set_var_typed_value(var, ASN_OCTET_STR,
							(const u_char *) retval, retval_len);
					}
					...
					case COLUMN_RHCNODERUNNINGSERVICESNUM:
					{
						long *retval;
						size_t retval_len = 0;
						retval = get_rhcNodeRunningServicesNum(data_context,
									&retval_len);
						snmp_set_var_typed_value(var, ASN_INTEGER,
							(const u_char *) retval, retval_len);
					}
					...
					case COLUMN_RHCNODERUNNINGSERVICESNAMES:
					{
						char *retval;
						size_t retval_len = 0;
						retval = get_rhcNodeRunningServicesNames(data_context,
									&retval_len);
						snmp_set_var_typed_value(var, ASN_OCTET_STR,
							(const u_char *) retval, retval_len);
					}
			...
			case MODE_SET_RESERVE1:
				...
				if (!ci) {
					...
					if (!data_context) {
						ci->data_context = rhcNodesTable_create_data_context(table_info->indexes);
						ci->new_row = 1;
					} else {
						ci->data_context = data_context;
					}
					netsnmp_oid_stash_add_data(&commitStorage,
						(oid *) ((char *) suffix + 1), suffix_len - 1, ci);
				}
				break;
